...
def _resolve_form_procedures_dir(project_root: Path) -> Path:
    ...
    candidates = [
        (base / "formlib" / "procedures").resolve(),
    ]
    if base.name == "runs":
        candidates.append((base.parent / "formlib" / "procedures").resolve())
    ...
    raise FileNotFoundError(
        f"Packaged procedures folder not found. Tried: {', '.join(str(p) for p in candidates)}\n"
        f"Create it and put your FORM procedures there, or set GLAS_FORM_PROCS."
    )
    return p
