...
def run(state: AppState, arg: str) -> None:
    ...
    ok_pairs = run_jobs(state.form_exe, jobs, max_workers=min(len(jobs), 4)) if jobs else True
    if not ok_pairs:
        return
    ...
    try:
        ...
        return
    ok_total = run_jobs(state.form_exe, [("TotalLO", tot["form_dir"], tot["driver"])], max_workers=1)
    if not ok_total:
        return
    ...
    ok_master = run_jobs(state.form_exe, [("Ioperator_master", master["form_dir"], master["driver"])], max_workers=1)
    if ok_master:
