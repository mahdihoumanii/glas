...
3. **SymmetryRelations.m** → PaVe conversion and master integral mapping → `Files/SymmetryRelations.m` and `../form/Files/SymmetryRelations.h`

All outputs auto-populate from `meta.json` kinematics. The `M0M1top` directory is preserved for IBP stage usage.
...
- `extract topologies` — 4-stage topology extraction with interactive parallel FORM execution
...
- The `M0M1top` directory (generated by `extract topologies` Stage 3) must be preserved for IBP reduction.
