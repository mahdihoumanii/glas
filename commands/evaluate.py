...
from glas.commands.common import AppState, MODES, clamp_jobs, parse_mode_and_flags
...
from glaslib.parallel import chunk_range_1based, run_jobs
from glaslib.paths import resolve_procedures_dir
...
def run(state: AppState, arg: str) -> None:
    ...
    if mode in ("lo", "nlo"):
        ...
        incdir = resolve_procedures_dir(state.config.root)
        ...
        if use_dirac:
            ...
            ok_dirac = run_jobs(state.config.form_exe, tasks, max_workers=info.get("jobs_effective", jobs_req))
            if ok_dirac:
    ...
    # mode == "mct"
    try:
        out = prepare_mass_ct(state.ctx, form_exe=state.config.form_exe, jobs=jobs_req)
    ...
    ok = run_jobs(state.config.form_exe, tasks, max_workers=out["jobs_effective"])
    if not ok:
        return
