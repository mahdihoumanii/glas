 ...
 *
 * MODIFIED: Added environment-aware Singular path resolution via SINGULARPATH
 *)
...
WriteFullSingularInput::usage="\
WriteFullSingularBasisInput[expr, dir] generates a Singular input file \"sinIn.sing\" in the directory dir which returns two output files, one that replaced ApartBasis which does the grobner's \
decomposition and the other reducedpoly.out which is the output of ApartReduced. The output is in terms of qis, we return the output {dens, qis} so a replacement rule can be generated;"
...
MultivariatePassToSingular::usage="\
MultivariatePassToSingular[Expr] performs the reduction in a Singular file i.e. only ApartBasis and ApartReduce is done within mathematica. \
Uses SINGULARPATH environment variable or SetSingularPath[] to locate Singular executable. \
MultivariatePassToSingular[Expr, SingularPath] uses the explicitly provided SingularPath.";

MultivariateApartSingular::usage="\
MultivariateApartSingular[Expr,SingularPath] preforms ApartBasis and ApartReduce fully in Singular . ";

ResolveSingularExe::usage = "ResolveSingularExe[] returns the absolute path to the Singular executable.";
SetSingularPath::usage = "SetSingularPath[path] sets the Singular executable path (file or directory).";
...
(* ============================================================
   Singular Path Resolution
   ============================================================ *)

(* Global variable for user-set Singular path *)
$SingularExe = None;

ResolveSingularExe[] := Module[
  {envPath, exePath, candidates, found},
  
  (* 1. Check user-set $SingularExe *)
  If[$SingularExe =!= None && FileExistsQ[$SingularExe],
    Return[$SingularExe]
  ];
  
  (* 2. Check SINGULARPATH environment variable *)
  envPath = Environment["SINGULARPATH"];
  If[envPath =!= $Failed && envPath =!= "",
    If[DirectoryQ[envPath],
      (* Directory: try Singular, singular *)
      candidates = {
        FileNameJoin[{envPath, "Singular"}],
        FileNameJoin[{envPath, "singular"}]
      };
  ...
  (* 3. Try Singular from PATH *)
  exePath = FindFile["Singular"];
  If[exePath =!= $Failed,
    Return[exePath]
  ];
  ...
  (* 5. If nothing found, throw descriptive error *)
  Message[ResolveSingularExe::notfound];
  $Failed
];

ResolveSingularExe::notfound = "Singular executable not found. Please set SINGULARPATH environment variable (e.g. export SINGULARPATH=/opt/homebrew/bin/Singular) or call SetSingularPath[\"/path/to/Singular\"] in Mathematica.";

SetSingularPath[path_String] := Module[
  {},
  If[!FileExistsQ[path] && !DirectoryQ[path],
    Message[SetSingularPath::notfound, path];
    Return[$Failed]
  ];
  $SingularExe = path;
  path
...
SetSingularPath::notfound = "Path does not exist: `1`";

(* ============================================================
   Original MultivariateApart Functions
   ============================================================ *)
...
WriteFormReduceInput[gb_,ord_,formdirectory_]:=Module[{monlist,formrules,patternmatch,pre,str,qstring,varstring,template},
  ...
  varstring ="s12,s13,s14,s23,s34,s45,s15,mt,ep,Epsilon,D,s,t,u,dM,gs,Pi,nl,msq,im,nh,ktsq, lambda,x,kt3";
...
ApartReduce[expr_, gb_, ord_, opts:OptionsPattern[]]:=Module[{n, p, var=Flatten[ord], t, p1, p2, p3, ordm,str,ret,outfile,dirname,runfilename},
  ...
  If[OptionValue[UseFormProgram],
    ...
    ret=RunProcess[{"/opt/homebrew/bin/form", runfilename}, ProcessDirectory->dirname];
...
(* ============================================================
   MultivariatePassToSingular \[LongDash] Modified for environment-aware path resolution
   ============================================================ *)

Clear[MultivariatePassToSingular]; 
MultivariatePassToSingular[exp_]:= MultivariatePassToSingular[exp, None];
MultivariatePassToSingular[exp_, singularPath_]:= Module[{expr, dens, qis, Abbrevs, ord, gb, SingularPath, tempDir, infile, outfile},
  (* Determine which Singular path to use *)
  If[singularPath === None || singularPath === Automatic,
    SingularPath = ResolveSingularExe[];
    If[SingularPath === $Failed, Return[$Failed]];,
    (* Use provided path directly *)
    If[!FileExistsQ[singularPath],
      Print["Warning: Provided SingularPath does not exist: ", singularPath];
    ];
    SingularPath = singularPath;
  ];
  
  {expr,dens,qis}=AbbreviateDenominators[exp];
  If[Length[qis]!=0, 
    Abbrevs = Table[qis[[i]] -> den[dens[[i]]], {i, Length[dens]}]; 
    ord=ApartOrder[dens,qis];
    (* Use temporary directory instead of NotebookDirectory *)
    tempDir = $ApartTemporaryDirectory;
    CreateDirectoryIfNeeded[tempDir];
    infile = FileNameJoin[{tempDir,"apartbasisin.sing"}];
    outfile = FileNameJoin[{tempDir,"apartbasisout.m"}];
    WriteSingularBasisInput[dens,qis,ord,tempDir];
    RunProcess[{SingularPath},"StandardOutput","\nexecute(read(\"" <> infile <> "\"));"];
    gb = Get[outfile];
    DeleteFile[outfile];
    DeleteFile[infile];
    CollectFlat[ApartReduce[expr,gb,ord,UseFormProgram-> True],qis]/. Abbrevs, 
    expr
  ]
]

(* ============================================================
   Original MultivariateApartSingular and PfdMat functions (unchanged)
   ============================================================ *)

Clear[MultivariateApartSingular]; 
MultivariateApartSingular[f_]:= Module[{redExp,dens, qis , gb, redPoly, repden,str,infile,outfile, vars, missing, redfile, fac, inv, ord, expr, Abbrevs,SingularPath = "/opt/homebrew/bin/Singular",dir,tempDir},
tempDir = $ApartTemporaryDirectory;
CreateDirectoryIfNeeded[tempDir];
{expr, fac, inv} = AbbreviateDenominators[f]; 
If[Length[inv]!=0,
Abbrevs = Thread[Rule[inv,den/@fac]];
ord = ApartOrder[fac,inv];
  infile=FileNameJoin[{tempDir,"sin.sing"}];
  outfile=FileNameJoin[{tempDir,"apartbasisout.m"}];
redfile=FileNameJoin[{tempDir,"sinOut.m"}];
  str=OpenWrite[infile];
vars = Variables[f];
missing = Complement[vars, Last[ord]];
 WriteLine[str, "ring myring=0,"];
  WriteLine[str, "("<>Riffle[ToString[CForm[#]]&/@Flatten[{ord, missing}],","]<>"),"];
  WriteLine[str, "("<>Riffle[("dp("<>ToString[Length[#]]<>")")&/@ord,","]<>");"];
  Do[WriteLine[str, "poly mygen"<>ToString[i]<>"=1-"<>ToString[InputForm[fac[[i]]*inv[[i]]]]<>";"],{i,Length[fac]}];
  WriteLine[str, "ideal myideal="<>Riffle[Table["mygen"<>ToString[i],{i,Length[fac]}],","]<>";"];
  WriteLine[str,"ideal mygb = slimgb(myideal);"];
  ...
  WriteLine[str,"write(outfile, \"{\", mygb, \"}\");"];
  WriteLine[str,"close(outfile);"];
WriteLine[str,"poly f = "<>ToString[InputForm[expr]]<>";"];
WriteLine[str,"poly reduced_f = reduce(f, mygb);"];
WriteLine[str,"link redfile = \"ASCII:w "<>redfile<>"\";"];
WriteLine[str,"write(redfile, reduced_f);"];
WriteLine[str,"close(redfile);"];
  WriteLine[str,"quit;"];
  Close[str];
RunProcess[{SingularPath},"StandardOutput","\nexecute(read(\"" <> infile <> "\"));"];
redExp = Get[redfile];
...
DeleteFile[outfile];
DeleteFile[infile];
CollectFlat[redExp, inv]/. Abbrevs
,f]]

SingularPath = "/opt/homebrew/bin/Singular";
PfdMat[exp_]:= Module[{ord,densS,dens,indexed,file, ToSing, ExpFile, expSing, Ready, int, int1, SingularPath=  "/opt/homebrew/bin/Singular",expr,fac,inv},
file = OpenWrite["SingularFile.sing"]; 
WriteString[file,"LIB \"pfd.lib\";\n"]; 
{expr, fac, inv} = AbbreviateDenominators[exp]; 
ord = ApartOrder[fac,inv]// Last;
If[Length[ord] != 0,
WriteString[file," ring r = 0,",StringReplace[ToString[ord],{"{"-> "(","}"-> ")"}],",dp;\n"];
WriteString[file, "pfdMat(\"exp.txt\",1,1,2,1);\n"];
WriteString[file, "quit;"];
Close[file];
Print["Wrote SingFile"]; 
ExpFile = OpenWrite["exp.txt"]; 
WriteString[ExpFile, "{",ToString[exp,InputForm],"}"];;
Close[ExpFile];
Print["Wrote Expr File"];
RunProcess[{SingularPath},"StandardOutput","\nexecute(read(\"SingularFile.sing\"));"];
indexed = ToExpression[Import["exp_pfd_indexed.txt", "Text"]]// Flatten; 
densS = ToExpression[StringReplace[StringJoin["{",StringReplace[Import["exp_denominator_factors.txt", "Text"],{"="-> "->",";"-> ",",WhitespaceCharacter-> ""}],"}"], ",}"-> "}"]];
dens = Table[densS[[i]][[1]]-> 1/den[densS[[i]][[2]]], {i, Length[densS]}];
DeleteFile["exp.txt"];
DeleteFile["exp_pfd.txt"];
DeleteFile["exp_denominator_factors.txt"];
DeleteFile["exp_pfdMat_logfile.txt"];
DeleteFile["exp_pfd_indexed.txt"];
indexed //. dens// CollectFlat[#, {den[__]}]&
,exp]
]
