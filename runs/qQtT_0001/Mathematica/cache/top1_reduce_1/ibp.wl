...
Put[ZeroSectors[top1],"results/zerosectors"];
If[NonZeroSectors[top1]==={}, Print["this family is trivial."];Quit[]];
...
rawmasters=<|BL[top1, {0, 0, 1, 1}] -> {BL[top1, {0, 0, 1, 1}]}, BL[top1, {0, 0, 0, 1}] -> {BL[top1, {0, 0, 0, 1}]}, BL[top1, {0, 0, 1, 0}] -> {BL[top1, {0, 0, 1, 0}]}|>;
GetSeeds["SubSym"][sector_]:=Lookup[rawmasters,sector,{}];
GetSeeds["ExtMap"][sector_]:=GenSeeds[sector,{0,maxdots+1},0,rank];
FastGenIds[top1,GetSeeds,"Directory"->"ibps","LaunchKernels"->4,"GetSectors"->GetSectors];
];
...
Export["ibps/ids_top1_exints.mx",(-#[[1]]+#[[2]])&/@exints,"MX"];
Export["ibps/ints_top1_exints.mx",IntegralsIn[exints],"MX"];
usints = {};
Export["ibps/usints_def.m",usints];
Export["ibps/ids_top1_usints.mx",(-#[[1]]+#[[2]])&/@usints,"MX"];
Export["ibps/ints_top1_usints.mx",IntegralsIn[usints],"MX"];


If[!({s12, s13, ep}==={}),SerializeFastIds[FileNames["ibps/ids_top1_*.mx"], {msq -> 1}, {s12, s13, ep}, "UserDefinedInts"->(First/@usints),"ExtraInts"->(First/@exints)]];
];
